...
/// A struct-like type with a field.
...
pub unsafe trait HasField<FieldTok>: Sized
where
    FieldTok: Copy,
{
    type FieldTy;

    unsafe fn field_raw(ptr: NonNull<Self>, _tok: FieldTok) -> NonNull<Self::FieldTy>;
    ...
    /// returned wand.
    // TODO: should require empty pointee pred
    fn get_field_virt<'this, 'field, PtrPerm>(
        self: VPtr<PtrPerm, Self>,
        _tok: FieldTok,
    ) -> ExistsLt!(<'sub> = (
            VPtr<PointsTo<'sub, PtrPerm::Access>, Self::FieldTy>,
            Wand<
                VPtr<PointsTo<'sub, PtrPerm::Access>, Self::FieldTy>,
                VPtr<PtrPerm, Self>
            >,
    ...
    /// out). The original permissions can be recovered by relinquishing the subpointer using the
    /// returned wand.
    fn get_field<'this, 'field, PtrPerm>(
...
/// The `FieldTok` is a token identifying the field, which allows for types with several fields.
/// Safety: TODO, definitely something about transmutability.
pub unsafe trait HasGenericPermField<FieldTok, FieldPerm>
where
    FieldTok: Copy,
    FieldPerm: PtrPerm,
    Self: ErasePerms,
    Self: HasField<FieldTok, FieldTy = Self::GenericFieldTy<FieldPerm>>,
{
    type GenericFieldTy<Perm: PtrPerm>;
    ...
    fn get_generic_field_virt<'this, 'field, PtrPerm, NewFieldPerm: self::PtrPerm>(
        self: VPtr<PtrPerm, Self>,
        _tok: FieldTok,
    ) -> ExistsLt!(<'sub> = (
            // What if there's a change I could do to an existential brand in FieldTy that invalidates
            // Self?
            VPtr<PointsTo<'sub, PtrPerm::Access>, Self::GenericFieldTy<FieldPerm>>,
            Wand<
                VPtr<PointsTo<'sub, PtrPerm::Access>, Self::GenericFieldTy<NewFieldPerm>>,
    ...
    /// returned wand.
    // TODO: could (should?) require empty pointee pred
    fn get_generic_field<'this, 'field, PtrPerm, NewFieldPerm: self::PtrPerm>(
        self: Ptr<PtrPerm, Self>,
        tok: FieldTok,
    ) -> ExistsLt!(<'sub> = (
            Ptr<PointsTo<'sub, PtrPerm::Access>, Self::GenericFieldTy<FieldPerm>>,
            Wand<
                VPtr<PointsTo<'sub, PtrPerm::Access>, Self::GenericFieldTy<NewFieldPerm>>,
                VPtr<PtrPerm, Self::ChangePerm<NewFieldPerm>>
            >,
        ...
        PtrPerm: HasRead<'this>,
    {
        let this = self.copy();
        self.into_virtual()
            .get_generic_field_virt::<PtrPerm, NewFieldPerm>(tok)
            .unpack_lt(|(field_vptr, wand)| {
                let field_ptr = unsafe { Self::field_raw(this.as_non_null(), tok) };
                let field_ptr = unsafe { Ptr::new_with_vptr(field_ptr, field_vptr) };
                ExistsLt::pack_lt((field_ptr, wand))
    ...
    FieldPerm: PtrPerm,
{
    ...
    where
        ...
        Self: HasGenericPermField<
            FieldTok,
            FieldPerm,
            GenericFieldTy<FieldPerm> = Option<Ptr<FieldPerm, Self::PointeeTy>>,
        >,
        Self: HasGenericPermField<
            FieldTok,
            FieldPerm,
            GenericFieldTy<PointsTo<'field>> = Option<Ptr<PointsTo<'field>, Self::PointeeTy>>,
        >,
    {
        let this = self.copy();
        self.get_generic_field(tok)
    ...
    where
        ...
        Self: HasGenericPermField<
            FieldTok,
            FieldPerm,
            GenericFieldTy<NewPerm> = Option<Ptr<NewPerm, Self::PointeeTy>>,
        >,
    {
        let this = self.copy();
        self.get_generic_field(tok)
    ...
    where
        ...
        >,
    {
        self.get_generic_field_virt(tok)
            .unpack_lt(|(ptr_to_field, wand)| {
